...
KANBAN_GUIDANCE = (
    ...
    "you MUST call `kanban_heartbeat` at least hourly** — the dispatcher may "
    "reclaim tasks past `kanban.dispatch_stale_timeout_seconds` (default 4h) "
    "with no heartbeat in the last hour.\n"
    "4. **Block only on real blockers.** Use `kanban_block(reason=\"...\")` "
    "only for human decisions or external dependencies you cannot resolve "
    "(credentials, UX choice, paywall, peer output). It is not a parking "
    "state; if a live session takes over, complete or create a handoff card.\n"
    ...
    "before counting as merged/done, put changed_files / tests_run / diff_path "
    "in a `kanban_comment` first, then end with "
    ...
    "7. **Failed audits stay visible.** For review/audit/QA, FAIL/not-ready "
    "must create an owner remediation card and complete with "
    "`created_cards=[...]`, or block for human decision.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
    ...
    "If your task title starts with `Coordinator review/unblock:`, you are "
    "the reviewer for the original blocked task. Read the review card, inspect "
    "the original task, and do not complete your review card until you have "
    "unblocked the original task, completed it, or blocked your review card "
    "with the remaining human-only decision.\n"
    "\n"
    ...
    "- **Orchestrating: discover profiles first.** Unknown assignees leave cards "
    "stuck in `ready`. Ground every assignee in a real profile "
    "(`hermes profile list`, or ask the user), and express dependencies via "
    "`parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
