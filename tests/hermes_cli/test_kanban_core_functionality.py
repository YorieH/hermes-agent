...
def test_gateway_dispatcher_disables_corrupt_board_without_traceback(
    ...
    # The zombie reaper probes the board once per tick (2 connections), and the
    # first tick attempts one dispatch connection. A failed dispatch has no
    # DispatchResult, so the result-aware health probe opens no extra connection
    # and cannot diagnose queued work without the tick's deferral context. The
    # second dispatch tick is quarantined before connect.
